...
def _init_default_admin(app):
    ...
    if not admin_role:
        ...
        try:
            ...
            new_menus = ['ai_chat', 'ai_sessions', 'skills', 'agent_manager', 'mcp_servers', 'open_api', 'cache_stats', 'business_systems', 'system_tasks', 'profit_share', 'tickets', 'system_map', 'pay_withdraw']
            updated = False
            for m in new_menus:
                if m not in menus:
                    menus.append(m)
