...
def sanitize_error_for_user(error_message: str, is_admin: bool) -> dict:
    ...
    clean = sql_pattern.sub('', raw)

    # 移除堆栈跟踪（Traceback, File "..."等）
    traceback_pattern = re.compile(r'(?:Traceback.*?)(?:\n\s+.*?)*', re.IGNORECASE | re.DOTALL)
    clean = traceback_pattern.sub('', clean)

    # 移除SQLAlchemy相关错误详情
    clean = re.sub(r'\(.*?\[SQL:.*?\]\).*?', '(数据库执行错误)', clean, flags=re.IGNORECASE)

    # 移除Python异常堆栈行（File "...", line N, in ...）
    file_pattern = re.compile(r'^\s*File\s+".*?"\s*,\s*line\s+\d+.*$', re.MULTILINE)
    clean = file_pattern.sub('', clean)

    # 移除以 Exception/Error 结尾的详细堆栈
    error_trace_pattern = re.compile(r'^\w+(?:Error|Exception):\s*.*$', re.MULTILINE)
    clean = error_trace_pattern.sub('', clean)
    ...
    return {'error_message': clean, 'ai_suggestion': ai_suggestion}
...
def _generate_error_suggestion(clean_error: str, raw_error: str) -> str:
    ...
    if 'syntax' in lower or 'sql' in lower or '语法' in lower:
