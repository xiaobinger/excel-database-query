...
### MCP 市场增强：Tab 切换 + 手动刷新 + 已引入标记 + 工具查看

**Tab 切换**：MCP 服务管理页面拆分为「我的 MCP 服务」和「MCP 市场」两个 Tab，不再使用弹窗展示市场，操作更直观。

**手动刷新市场**：市场 Tab 页顶部新增「刷新市场」按钮，可手动刷新市场目录并同步已引入状态。
...
| 日期 | 版本 | 内容 |
|------|------|------|
| 2026-09-02 | v2.5.7 | MCP 市场增强：Tab 切换（我的服务/市场）、手动刷新市场、已引入标记（绿色标签+禁用按钮）、工具查看按钮（直接查看工具清单 mcp__{服务名}__{工具名}）；新增工具刷新时间列、删除确认弹窗、创建/删除/导入后自动同步市场状态；后端新增 `POST /api/mcp/marketplace/refresh` 接口 |
